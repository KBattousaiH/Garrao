...
%
% Mejoras Garrao
load XYZ_GARRAO_A3_ajustelatlon.mat
%
% Etopo2 Garrao 
etopo_garrao='Bati_Garrao_A3_v2.nc';
...
rang=[min(min(XYZ_GARRAO_A3_corte_ajustelatlon(:,3))) max(max(XYZ_GARRAO_A3_corte_ajustelatlon(:,3)));...
...
figure()
subplot(2,2,1)
scatter3(-XYZ_GARRAO_A3_corte_ajustelatlon(:,1),XYZ_GARRAO_A3_corte_ajustelatlon(:,2),XYZ_GARRAO_A3_corte_ajustelatlon(:,3),[],XYZ_GARRAO_A3_corte_ajustelatlon(:,3),'.')
colormap parula
%colorbar
view(0,90)
xlim([Xlim(1) Xlim(2)])
...
m_gshhs_f('patch',[.7 .7 .7],'EdgeColor','k')
m_grid('linewi',2,'tickdir','out','fontsize',10); % Tamaño nº coordenadas
caxis(rango) %caxis([0 85])
title('A3 Etopo Garrao')
cmocean('deep')
set(get(cb3,'title'),'string','[m]');
saveas(f1,'A3_Etopo_Garrao','png');
%
f2=figure();
m_proj('equidistant','lon',[min(min(Xlim)) max(max(Xlim))],'lat',[min(min(Ylim)) max(max(Ylim))]);
...
caxis([-100 100])
cmocean('balance')
title('A3 Etopo-GRD Garrao')
saveas(f4,'A3_Etopo_-_GRDGarrao','png');
%
f5=figure();
m_proj('equidistant','lon',[min(min(Xlim)) max(max(Xlim))],'lat',[min(min(Ylim)) max(max(Ylim))]);
...
caxis([-100 100]) % 300
cmocean('balance')
title('A3 Etopo-GRD CROCO')
saveas(f5,'A3_Etopo_-_GRDCROCO','png');
%
f6=figure();
m_proj('equidistant','lon',[min(min(Xlim)) max(max(Xlim))],'lat',[min(min(Ylim)) max(max(Ylim))]);
...
caxis([-50 50])
cmocean('balance')
title('A3 GRD Garrao-GRD CROCO')
saveas(f6,'A3_GRDGarrao_-_GRDCROCO','png');
%
